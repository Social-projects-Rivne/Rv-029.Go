...
func (user *User)FindByID(id string){
	...
	defer db.Session.Close()
	if err := db.Session.Query(`SELECT id, email, first_name, last_name, password, salt, role, created_at, update_at FROM users WHERE id = ? LIMIT 1`,
...
func (user *User)FindByEmail(email string){
	...
	defer db.Session.Close()
	if err := db.Session.Query(`SELECT id, email, first_name, last_name, password, salt, role, created_at, update_at FROM users WHERE email = ? LIMIT 1`,
